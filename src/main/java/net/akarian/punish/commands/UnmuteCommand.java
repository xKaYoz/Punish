        ...
        if (sender.hasPermission("punish.unmute")) {
            if (args.length == 1 || args.length == 2) {
                ...
                if(PunishmentHandler.isMuted(uuid) == 1){
                    ...
                    if (!isSilent) {
                        Chat.broadcast(lang.getString("Unmute Message").replace("$player$", name).replace("$staff$", sender.getName()));
                    } else {
                        for (Player p : Bukkit.getOnlinePlayers()) {
                            if (p.hasPermission("punish.silent")) {
                                Chat.sendRawMessage(p, lang.getString("Silent Unmute Message").replace("$player$", name).replace("$staff$", sender.getName()));
                            }
                        }
                    }

                } else {
                    Chat.sendMessage(sender, "&7That player is not currently muted.");
                }
            } else {
                Chat.usage(sender, "unmute <player>");
            }
        } else {
