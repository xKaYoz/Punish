    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (sender.hasPermission("punish.unmute")) {
            if (args.length >= 1) {
                ...
                if (args.length == 1) {
                    reason = new StringBuilder(config.getString("DefaultWarnReason"));
                } else {
                ...
                switch (PunishmentHandler.warn(uuid, staff, 60 * 60 * 24 * 90, trim)) {
                    case 0:
                        ...
                        if (!isSilent) {
                            Chat.broadcast(lang.getString("Warn Message").replace("$player$", name).replace("$reason$", trim).replace("$staff$", sender.getName()));
                        } else {
                            Chat.sendRawMessage(Bukkit.getConsoleSender(), lang.getString("Silent Warn Message").replace("$player$", name).replace("$reason$", trim).replace("$staff$", sender.getName()));
                            for (Player p : Bukkit.getOnlinePlayers()) {
                                if (p.hasPermission("punish.silent")) {
                                    Chat.sendRawMessage(p, lang.getString("Silent Warn Message").replace("$player$", name).replace("$reason$", trim).replace("$staff$", sender.getName()));
                                }
                            }
                        }
                        break;
                ...
                Chat.usage(sender, "warn <player> <reason>");
            }
        } else {
