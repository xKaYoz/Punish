    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender.hasPermission("punish.mute")) {
            if (args.length >= 1) {
                        ...
                        if (!isSilent) {
                            ...
                            for (Player p : Bukkit.getOnlinePlayers()) {
                                if (p.hasPermission("punish.silent")) {
                                    Chat.sendRawMessage(p, lang.getString("Silent Mute Message").replace("$player$", name).replace("$reason$", trim).replace("$staff$", sender.getName()));
                                }
                            }
                        }
                        break;
                ...
                Chat.usage(sender, "mute <player> {reason}");
            }
        } else {
