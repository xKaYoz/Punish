...
public class BlacklistCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        if (sender.hasPermission("punish.blacklist")) {
            if (args.length > 0) {
                if (args[0].replace(".", ",").split(",").length == 4) {
                    ...
                    if (PunishmentHandler.isIPLogged(uuid.toString())) {
                        ...
                        switch (PunishmentHandler.blacklist(ip, staff, reason.toString(), isSilent)) {
                            case (0):
                                String trim = reason.toString().replace("-s", "").trim();
                                if (!isSilent) {
                ...
                Chat.usage(sender, "blacklist <player/ip> {reason}");
            }
        } else {
